...
void setData()
{
  ...
  // publish zenoh
  zenoh.publish(KEY_WATERMAKER_MAIN_BOOSTPUMP_TEMPERATURE, floor(boostTemp));
  zenoh.publish(KEY_WATERMAKER_MAIN_HIGHPRESSUREPUMP_TEMPERATURE, floor(hpTemp));
  zenoh.publish(KEY_WATERMAKER_MAIN_PREFILTERPRESSURE, floor(preFilterPressure.getAverage() * MPA_TO_PA));
  zenoh.publish(KEY_WATERMAKER_MAIN_POSTFILTERPRESSURE, floor(postFilterPressure.getAverage() * MPA_TO_PA));
  zenoh.publish(KEY_WATERMAKER_MAIN_PREMEMBRANEPRESSURE, floor(postFilterPressure.getAverage() * MPA_TO_PA));
  zenoh.publish(KEY_WATERMAKER_MAIN_POSTMEMBRANEPRESSURE, floor(postMembranePressure.getAverage() * MPA_TO_PA));
  zenoh.publish(KEY_WATERMAKER_MAIN_PRODUCTSALINITY, floor(tdi.getAverage()));
  zenoh.publish(KEY_WATERMAKER_MAIN_STARTTIME, startTime * 1000.0);
  zenoh.publish(KEY_WATERMAKER_MAIN_STOPTIME, stopTime * 1000.0);
  if (running)
  {
    zenoh.publish(KEY_WATERMAKER_MAIN_STATUS, "RUNNING");
    zenoh.publish(KEY_WATERMAKER_MAIN_RUNTIME, floor((rtc.getLocalEpoch() - startTime))); // seconds
  }
  else
  {
    zenoh.publish(KEY_WATERMAKER_MAIN_STATUS, "STOPPED");
    zenoh.publish(KEY_WATERMAKER_MAIN_RUNTIME, floor((stopTime - startTime))); // seconds
  }
}
...
void setup()
{
  ...
  //setup zenoh
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_BOOSTPUMP_TEMPERATURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_HIGHPRESSUREPUMP_TEMPERATURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_PREFILTERPRESSURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_POSTFILTERPRESSURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_PREMEMBRANEPRESSURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_POSTMEMBRANEPRESSURE);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_PRODUCTSALINITY);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_STARTTIME);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_STOPTIME);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_STATUS);
  zenoh.declarePublisher(KEY_WATERMAKER_MAIN_RUNTIME);
